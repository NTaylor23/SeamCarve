...
class ImageWorker():
    ...
    def process(self, scaling_factor: int) -> None:
        ...
        if self.curr_scaling_factor > self.prev_scaling_factor and \
                new_dim in self.seam_cache:
            ...
            seam = list(map(lambda x: x[0][1] if x else 0,
                            self.seam_cache[new_dim]))
        else:
    ...
    def add_seam(self, scaling_factor):
        ...
        new_image = np.zeros((height, width + 1, channels),
    ...
    def calculate_energies(self, grid: np.ndarray) -> np.ndarray:
        ...
        for row in reversed(range(grid.shape[0] - 1)):
            min_energy = np.minimum(grid[row + 1, :-2], grid[row + 1, 1:-1])
            min_energy = np.minimum(min_energy, grid[row + 1, 2:])
            grid[row, 1:-1] += min_energy
            grid[row, 0] += min(grid[row + 1, 0], grid[row + 1, 1])
            grid[row, -1] += min(grid[row + 1, -2], grid[row + 1, -1])
    ...
    def define_path(self, grid: np.ndarray):
        ...
        column_ranges = [range(max(0, col - 1), min(width, col + 2))
                         for col in range(width)]

        for row in range(1, height - 1):
            next_x = col
            for k in column_ranges[col]:
        ...
        self.seam_cache[(height, width)] = pixel_layer
...
thread = threading.Thread(target=start_queue, daemon=True)
thread.start()
